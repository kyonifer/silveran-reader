...
public enum CoverColorAverager {
    ...
    public static func surfaceColor(rgbaPixels: [UInt8], dark: Bool) -> CoverRGB8 {
        let surface = averageColor(rgbaPixels: rgbaPixels).map { average in
            let source = hsv(
        ...
        } ?? HSV(hue: 0.56, saturation: 0.34, brightness: 0.28)
        ...
        return rgb(HSV(hue: surface.hue, saturation: lightSaturation, brightness: 0.92))
    }
    ...
    private static func hsv(red: Double, green: Double, blue: Double) -> HSV {
