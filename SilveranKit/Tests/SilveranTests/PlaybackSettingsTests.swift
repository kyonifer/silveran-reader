...
@Test func legacyPlaybackSpeedMigratesToBothListeningAndReadaloudSpeeds() throws {
    let data = Data(
        """
        {
          "playback": {
            ...
            "defaultVolume": 0.8,
            "statsExpanded": true,
            "lockViewToAudio": false
          }
        }
        """.utf8
    )

    let config = try JSONDecoder().decode(SilveranGlobalConfig.self, from: data)

    #expect(config.playback.defaultPlaybackSpeed == 1.75)
    #expect(config.playback.readaloudPlaybackSpeed == 1.75)
}
...
@Test func legacyPlaybackSpeedsAreNormalizedToTheSupportedRange() throws {
    let data = Data(
        """
        {
          "playback": {
            "defaultPlaybackSpeed": 10.0
          }
        }
        """.utf8
    )
    ...
    #expect(config.playback.defaultPlaybackSpeed == kMaximumPlaybackSpeed)
    #expect(config.playback.readaloudPlaybackSpeed == kMaximumPlaybackSpeed)
}
...
@Test func persistedPlaybackSpeedsCannotBypassTheCouplingInvariant() throws {
    let data = Data(
        """
        {
          "playback": {
            "defaultPlaybackSpeed": 3.0,
            "readaloudPlaybackSpeed": 1.5
          }
        }
        """.utf8
    )
    ...
    #expect(config.playback.defaultPlaybackSpeed == 1.5)
    #expect(config.playback.readaloudPlaybackSpeed == 1.5)
}
